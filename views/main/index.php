      ...
      <?php endforeach; ?>
        ...
        <div class="recommended-goods">
          ...
          <h1>Рекомендуемые товары</h1>
           ...
           <?php foreach ($recommendedGoods as $book): ?>
            <div class='prod-box'>
              <a href='/good/<?=$book['id']?>'>
                <div class='prod-category'>
                  <object>
                    <a href="/category/<?=$book['category']?>" title='<?=$book['category']?>'><?=$book['category']?></a>
                  </object>
                </div>
                <div class='prod-img'> 
                  <img src='<?=$book['path']?>' alt='' border='0'/>
                  <?php if ($book['is_discount']):?>
                    <div id="discount2"><img src='\template\picture\icon\discount2.png'/></div>
                  <?php endif;?>
                  <?php if ($book['is_special']):?>
                    <div class="hot"><img src='\template\picture\icon\special.png'/></div>
                  <?php endif;?>
                </div>
                <div class='prod-title'>
                  <object>
                    <a href='/good/<?=$book['id']?>' title='<?=$book['title']?>'><?=$book['title']?></a>
                  </object>
                </div>
                
                <div class='prod-author'>
                  <span title='Автор'> <?=$book['author']?></span>      
                </div>
              </a>
              <div class='clearfix'>
                <div class='prod-price'>
                 <?php if (!$book['newprice']):?>          
                  <span class='price'><?=$book['price'].' грн'?></span>
                 <?php  else:?>
                  <div class = 'old-price'><span class='reduce'><?=$book['price'].' грн'?></span></div>
                  <div class = 'new-price'><span id='hot-price'><?=$book['newprice'].' грн'?></span></div>
                 <?php endif;?>
                </div>

                <div class='order'><?=$book['status']?></div>
              </div>
            </div>
           <?php endforeach; ?>        
        </div>
